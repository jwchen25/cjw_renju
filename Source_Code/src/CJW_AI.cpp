...
void CJW_AI::getPosition(char chesspadstate[15][15], int color)  //计算下一步棋子的位置
{
	int AI_Color;  //AI棋子的颜色
	int Competitor_Color;  //对手棋子的颜色
	Position Optimal_Position;
	if(color==1) AI_Color=2;
	else AI_Color=1;
	...
	for(int i=0; i<15; i++)
    {
		for(int j=0; j<15; j++)
		{
			if(chesspadstate[i][j]=='1'||chesspadstate[i][j]=='3')
				tempstate[i][j]=2;
			else if(chesspadstate[i][j]=='2'||chesspadstate[i][j]=='4')
				tempstate[i][j]=1;
		}
	}
	...
	for (int i = 0;i < 15;i++)
		for (int j = 0;j < 15;j++) {
			...
			position.y = j;
			//我的分数
			score = judgeChessSituation(tempstate, position, AI_Color);  //返回当前形势分数
			myscore[i][j] = score;

			if (AI_Color == BLACKFLAG)
				Competitor_Color = WHITEFLAG;
			else
				Competitor_Color = BLACKFLAG;
			//对手分数
			score = judgeChessSituation(tempstate, position, Competitor_Color);  //返回当前形势分数
			hisscore[i][j] = score;
		}

	//根据分数，给出位置
	Optimal_Position=maxScore(myscore, hisscore);
	AI_Piece_x=Optimal_Position.x;
	AI_Piece_y=Optimal_Position.y;

}

int CJW_AI::judgeChessSituation(const int state[15][15], Position position, int color) {
...
int CJW_AI::giveScore(Situation situation) {
...
int CJW_AI::getType(const int state[15][15], Position position, int color, int direction) {
...
void CJW_AI::getChess(int chess[9], const int state[15][15], Position position, int color, int direction) {
...
int CJW_AI::judgeType(const int chess[9]) {
	int mycolor = chess[4];
	int hiscolor;
...
Position CJW_AI::maxScore(const int myscore[15][15], const int hisscore[15][15]) {
